...
import {
  ...
  MessageSquareText, Search, ChevronsDown,
} from 'lucide-react';
...
import { SHOW_REWARDS } from '@/lib/flags';
...
export function CarveMarketingPage() {
  return (
    <div className="min-h-screen w-full bg-[#0A0A0B] text-white">
      {/* Hero Section */}
      <section className="min-h-[100dvh] flex flex-col items-center justify-center px-6 pt-20 pb-28 relative">
        {/* @ai-why: `alt=""` en niet "Carve logo" — het woordmerk eronder zegt al
            hetzelfde, en een schermlezer die beide voorleest herhaalt zichzelf. */}
        <motion.div
        ...
        {/* @ai-why: Vertraging 1.9s omdat de ScoreboardCard pas op 1.5s binnenkomt en
            tot 2.2s beweegt; een knop die daar middenin verschijnt vecht om de blik.
            De knop staat in de flow (niet absoluut) zodat hij vanaf de eerste frame
            ruimte inneemt en de kaart niet omhoog springt als hij zichtbaar wordt. */}
        <motion.a
          href={APP_STORE_URL}
          onClick={() => track('app_store_click', { source: 'hero' })}
          target="_blank"
          rel="noopener noreferrer"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, delay: 1.9 }}
          className="mt-10 inline-flex items-center gap-2.5 rounded-xl bg-white px-6 py-3.5 text-sm font-semibold text-black transition-colors hover:bg-white/90"
        >
          <AppleGlyph className="w-4 h-4" />
          Download on the App Store
        </motion.a>

        {/* @ai-why: Een anker en geen scroll-handler: `scroll-behavior: smooth` staat
            al in globals.css, dus dit werkt ook voordat React gehydrateerd is en het
            geeft de bezoeker een echte link. Absoluut onderaan met `inset-x-0
            flex justify-center` in plaats van `-translate-x-1/2`, omdat framer-motion
            op een geanimeerd element de transform overschrijft. */}
        <div className="absolute inset-x-0 bottom-8 flex justify-center">
          ...
          >
            ...
            >
              <ChevronsDown className="w-7 h-7" strokeWidth={1.5} />
            </motion.span>
          </motion.a>
        </div>
      </section>

      {/* @ai-sync: het scroll-icoontje in de hero hierboven wijst met `href="#content"`
          naar deze id. Hernoem je hem, hernoem dan ook dat anker. */}
      <div id="content">
      ...
      <section className="py-24 md:py-32 px-6">
        ...
        {/* @ai-why: Stond tot 2026-09-05 op "Health. Money. Travel." met kaarten voor
            Money en Travel. Allebei zijn ze in de iOS-app achter een vlag gezet
            (FeatureFlags.showMoneyTab, showLegacyLifeTab), dus die kaarten beloofden
            schermen die je na het downloaden niet vindt. De drie hieronder zijn de
            app zoals hij vandaag is, in dezelfde volgorde als de App Store-listing.
            @ai-sync: ~/Developer/Carve-AI/docs/marketing/app-store-listing.md
            @ai-sync: ~/Developer/Carve-AI/Carve AI/App/Config/FeatureFlags.swift */}
        <div className="max-w-4xl mx-auto grid md:grid-cols-3 gap-4">
          {[
            { icon: <Camera className="w-5 h-5" />, title: 'Photo in, macros out', text: 'Take a picture of your plate. The AI works out the calories and macros. No database digging.' },
            { icon: <Dumbbell className="w-5 h-5" />, title: 'See what you skip', text: 'The muscle map fills where you trained and stays empty where you did not. Legs included.' },
            { icon: <MessageSquareText className="w-5 h-5" />, title: 'A coach that remembers', text: 'Type what you ate or trained. It knows your week, your protein and your recovery.' },
          ].map((card, i) => (
      ...
      {/* @ai-why: De rewards-sectie verkoopt XP, rangen en "unlock Pro days". Dat hele
          systeem hangt in de app achter FeatureFlags.showProDaysRewards, en die staat
          uit: verdiende dagen komen in de tier-resolver wél als getal terug maar geven
          geen Pro (zie de @ai-todo op die vlag). Een beloning adverteren die niet wordt
          uitgekeerd is erger dan hem verzwijgen. Een constante en geen weggehaald blok,
          zodat de sectie meecompileert en in één regel terug kan.
          @ai-sync: ~/Developer/Carve-AI/Carve AI/App/Config/FeatureFlags.swift (showProDaysRewards) */}
      {SHOW_REWARDS && <RewardsSection />}
      ...
      <div className="max-w-3xl mx-auto grid md:grid-cols-2 gap-4">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-6 h-full flex flex-col">
            ...
            <div className="px-3 py-2 rounded-lg bg-[#4D99F2]/10 border border-[#4D99F2]/20 mb-6">
              <p className="text-[12px] text-[#4D99F2] font-medium">
                ...
                30 days Pro free on sign-up
              </p>
            </div>
            <p className="text-3xl font-bold text-white mb-1">€0</p>
            <p className="text-white/30 text-sm mb-8">forever</p>
            <div className="space-y-3 flex-1">
              <FeatureRow included label="30-day Pro trial included" />
        ...
        <ScrollReveal animation="fade-up" delay={0.1}>
          <div className="rounded-2xl border border-[#D4A843]/30 bg-[#D4A843]/[0.04] p-6 h-full flex flex-col relative">
            ...
            <p className="text-[11px] text-[#4D99F2] font-medium mb-5">Includes 30-day free trial</p>
            ...
            <a href={APP_STORE_URL}
              onClick={() => track('app_store_click', { source: 'pricing' })} target="_blank" rel="noopener noreferrer" className="mt-8 px-5 py-3 bg-white text-black rounded-xl font-semibold text-sm text-center block hover:bg-white/90 transition-colors">
              Download on the App Store
            </a>
          </div>
        </ScrollReveal>
