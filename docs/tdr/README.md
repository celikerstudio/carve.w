...
| [0011](./0011-de-cockpit-bedient-de-campagnes.md) | De cockpit bedient de campagnes | Twee knoppen (pauzeren/hervatten, dagbudget) plus een kaal wijzigingsformulier, alle drie met een verplichte verwachting die als `wijziging` het journaal in gaat. Schrijven leeft in `lib/admin/meta-write.ts` achter een eigen `META_WRITE_TOKEN`, en het leestoken moet eerst zijn schrijfscope kwijt. Meta eerst, daarna teruglezen, dan pas het journaal. Geen automatisch ingrijpen. | Voorgesteld 2026-09-10 |
